...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ##
        self.updateLights() # Update lights when entering heat
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == 'heat'):
            if temp < self.setPoint:
                redLight.pulse() # Fading if below setpoint
            else:
                redLight.on()    # Solid if reached
        
        elif(self.current_state.id == 'cool'):
            if temp > self.setPoint:
                blueLight.pulse() # Fading if above setpoint
            else:
                blueLight.on()    # Solid if reached
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if((counter % 30) == 0):
                ##
                ser.write(self.setupSerialOutput().encode()) # Send to Serial Port
